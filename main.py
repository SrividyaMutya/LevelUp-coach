...
On Render (or any host): set GEMINI_API_KEY as an environment variable in
the dashboard — never hardcode the key in this file, since this repo is public.
"""
...
# ---------- API Routes ----------
# NOTE: all API routes are defined BEFORE the static file mount at the
# bottom of this file. FastAPI/Starlette match routes in the order they're
# registered, so anything under /api/... must be declared before the
# catch-all StaticFiles mount on "/", or the mount will shadow these routes.
...
# Serves index.html, app.js, styles.css directly from the repo root so one
# deployment (e.g. one Render service) hosts both the frontend and the API.
# This MUST come after every @app.get/@app.post route above — StaticFiles
# with html=True acts as a catch-all mount on "/", so anything registered
# after this point would be unreachable.
app.mount("/", StaticFiles(directory=os.path.dirname(__file__) or ".", html=True), name="frontend")
